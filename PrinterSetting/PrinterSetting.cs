...
internal class PrinterSetting
{
    ...
    internal static void SetPrinterSetting(string printerName, PageSetting printerSetting)
    {
        ...
        try
        {
            ...
            var result = NativeMethods.SetPrinter(hPrinter, 9, printerInfo, 0);
            if (!result)
    ...
    private static DevMode GetPrinterSettings(string printerName, out IntPtr yDevModeData, out IntPtr pi, out IntPtr hPrinter, out PrinterDefaults printerValues, out PrinterInfo9 pinfo)
    {
        ...
        var openPrinterResult = NativeMethods.OpenPrinter(printerName, out hPrinter, ref printerValues);
        if (!openPrinterResult)
            throw new Win32Exception(Marshal.GetLastWin32Error());

        NativeMethods.GetPrinter(hPrinter, 2, IntPtr.Zero, 0, out int nBytesNeeded);
        if (nBytesNeeded <= 0)
            throw new Exception("Unable to allocate memory");
        else
        {
            pi = Marshal.AllocCoTaskMem(nBytesNeeded);
            var getPrinterResult = NativeMethods.GetPrinter(hPrinter, 9, pi, nBytesNeeded, out _);
            if (!getPrinterResult)
                throw new Win32Exception(Marshal.GetLastWin32Error());
            ...
            if (pinfo.pDevMode == IntPtr.Zero)
            {
                ...
                var sizeOfDevMode = NativeMethods.DocumentProperties(IntPtr.Zero, hPrinter, printerName, ptrZero, ref ptrZero, 0);
                IntPtr ptrDM = IntPtr.Zero;
                try
                {
                    ptrDM = Marshal.AllocCoTaskMem(sizeOfDevMode);
                    int i = NativeMethods.DocumentProperties(IntPtr.Zero, hPrinter, printerName, ptrDM, ref ptrZero, DM_OUT_BUFFER);
            ...
            var intError = NativeMethods.DocumentProperties(IntPtr.Zero, hPrinter, printerName, IntPtr.Zero, ref temp, 0);
            yDevModeData = Marshal.AllocCoTaskMem(intError);

            _ = NativeMethods.DocumentProperties(IntPtr.Zero, hPrinter, printerName, yDevModeData, ref temp, 2);
    ...
    private class NativeMethods
    {
        ...
        internal static extern bool OpenPrinter([MarshalAs(UnmanagedType.LPWStr)] string szPrinter, out IntPtr hPrinter, ref PrinterDefaults pd);
        ...
        internal static extern int DocumentProperties(IntPtr hwnd, IntPtr hPrinter, [MarshalAs(UnmanagedType.LPWStr)] string pDeviceName, IntPtr pDevModeOutput, ref IntPtr pDevModeInput, int fMode);
